...
void slave_process_type1(uint64_t _pool_size, uint64_t _dimension, uint64_t numper, uint64_t _job_size,
	...
	MPI_Status status;
	uint64_t job_size, i; // job_size is the number of p-values
	int myid;
...
#pragma omp parallel for \
		...
		for (i = 0; i < _job_size; ++i) {
			...
			/* get the Z */
			double * v_Z = &pre_mvrnorm[gsl_rng_uniform_int(v_r[tid], _pool_size) * _dimension];

			uint64_t j, k, l, i_select = 0, *p_P0, start_pos, end_pos;
			double * p, *p_Z;
			...
			for (j = 0; j <= numper; ++j) {
				...
				/* sort the value for each trait */
				for (k = 0; k < _num_traits; ++k) {
					start_pos = _num_SNPs_per_gene * k;
					end_pos = start_pos + _num_SNPs_per_gene;
					sort(v_p_P0[tid], v_p_P0[tid] + start_pos);

					p_P0 = v_p_P0[tid];
					p_P0 += start_pos;
					p_Tper->at(start_pos).at(j).first = _log_array[*p_P0];
					p_Tper->at(start_pos).at(j).second = j;
					for (l = 1 + start_pos; l < end_pos; ++l) {
			...
			/* for each trait */
			for (l = 0; l < _num_traits; ++l) {
				start_pos = _num_SNPs_per_gene * l;
				end_pos = start_pos + _num_SNPs_per_gene;
...
void master_process_type1(uint64_t _num_replica, uint64_t _job_size, string _s_outfile, uint64_t _num_traits) {
	...
	uint64_t i, j, workloadSize, iBounderOfProcess, currentWorkId;
	ofstream outfile;
	...
	int count = _job_size * _num_traits;
	...
	double * p_pvalue_slave = new double[count], *p;
...
int main_afc_type1(int argc, char *argv[]) {
	...
	uint64_t num_traits = 1;
	uint64_t num_SNPs_per_gene = 1;
	...
	for (i = 1; i <= (uint64_t) vsArguments.size() - 1; i++) {
		if (vsArguments[i] == "-o") {
			...
			num_SNPs_per_gene = atoi(vsArguments[++i].c_str());
		}
	}
	...
	if(dimension != num_traits * num_SNPs_per_gene){
		cout << "Error: covariance matrix is not correct!" << endl;
		return 0;
	}
	...
	if (myid == 0) {
		...
		master_process_type1(num_replica, job_size, s_outfile, num_traits);
		...
		cout << "Slave " << myid << " initialization is done." << endl;
		slave_process_type1(pool_size, dimension, numper_permutations, job_size, pre_mvrnorm, log_array, num_traits,
				num_SNPs_per_gene);
		delete[] pre_mvrnorm;
		delete[] log_array;
